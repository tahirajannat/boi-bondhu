...
export default function Filter({ items, startIndex, endIndex }) {
    ...
    const handleCategoryChange = (event) => {
        ...
        setSelectedCategories(
            (prevCategories) =>
                checked
                    ? [...prevCategories, value] // add to selected categories
            ...
            {slicedBooks
                .filter((item) =>
                    selectedCategories.length === 0
                        ? true // If no categories are selected, show all items
                        : selectedCategories.includes(item.category)
                )
                .map((item, index) => (
                    <div className='shadow mb-8' key={index}>
